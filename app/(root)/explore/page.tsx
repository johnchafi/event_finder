...
import NoEventCard from "@/components/ui/noEvent-card"
...
export default async function EventsPage({searchParams} : SearchParamProps) {
  ...
  return (
    <div className="min-h-screen w-full bg-black text-white py-8">
      {/* Mobile Header */}
      ...
      <div className="flex flex-col lg:flex-row">
        ...
        {/* Main Content */}
        <div className="flex-1 p-4 wrapper">
          {/* Navigation */}
          <div className="flex md:flex-row flex-col  sm:items-center sm:justify-between gap-4 mb-8">
            ...
            <Search />
            <Button variant="ghost" className="lg:flex items-center gap-2">
              Reset Filters
            </Button>
          </div>
          ...
          <div>
            ...
            {
              activeEvents.length > 0 ? <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6">
              {
                    activeEvents.map((activeEv: any, i: any)=> (
                      <EventCard
                      key={"activeEV" + i}
                      event={activeEv}
                      limit={6}
            ...
            <h2 className="text-xl font-semibold mb-6 text-center my-8">
              PAST <span className="text-primary-800">EVENTS</span>
            </h2>
            {
              pastEvents.length > 0 ? <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6">
              {
                pastEvents.map((pastEv: any, i:any)=> (
                  <EventCard
                  key={"pastEv" + i}
                  event={pastEv}
                  limit={6}
